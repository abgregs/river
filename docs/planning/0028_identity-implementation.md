...
**Landed 2026-09-17**, on `feat/identity` (renamed from `docs/identity-studies`, which carried the design record). What shipped differs from the plan below in six places, each decided during the build or the maintainer's smoke:

1. **Assets are generated by checked-in tests, not a standalone script.** SwiftPM has no asset pipeline for a hand-rolled bundle, and `Bundle.module` resolves to the `.app` root, which breaks signing. `MenuBarGlyphTests` and `AppIconTests` render the PNGs and the `.icns` from the geometry in `Constants` under an env gate; a normal test fails if the checked-in files drift from it, `make bundle` copies them, and `make verify` asserts they ship.
2. **An app icon ships** — out of scope above, reopened by the maintainer when Finder showed the generic placeholder. The approved slat mark in ink on a charcoal squircle; no accent yet. `Info.plist` gains `CFBundleIconFile`, so AC8's "plist untouched" no longer holds.
3. **The glyph is an 18 pt box, not 16**, with wider rows and a pitch of 3.5. At the studies' size it read optically smaller than neighboring status items and sat a point low.
4. **The input level reference dropped from 0.2 to 0.08 RMS.** A browser applies automatic gain control to its microphone and `AVAudioEngine` does not, so the study page's `rms / 0.2` mapping under-read the same voice on device: loud speech reached only level 0.3. The indicator's own tuned numbers are untouched.
5. **The panel is one fixed size and position**, with content pinned to its top and room always reserved for the message rectangle — after a cancel shifted the capsule visibly.
6. **The 0017 canceled-notice bleed is fixed** (a notice no longer survives into the next recording), and feedback sounds now default **off**, matching "quiet by default".
