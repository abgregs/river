...
The rest state now is a calmer 0.5: amplitude and stroke width have a floor at the level-0.35 values (70% of 0.5's amplitude, stroke just under 0.5's width), the drift runs at 45% speed at rest and speeds up smoothly with level, and the ink dims to 44% with a slow breath of plus or minus 6% over about seven seconds (the breath's low point measures about 3.15:1 on charcoal; it holds still under Reduce Motion). Ink is the only thing that says "resting"; the water never stops. The amplitude floor is about 2.7 px on the thinnest strand, which keeps every strand out of the sub-pixel band that caused the grain. Ink follows the level through a smoothstep over the first half of the range, so on release it settles in step with the motion (the first build blended ink only below level 0.15, which the maintainer saw as a late, abrupt opacity step). Transcribing holds the level at 0.4 and, since 2026-09-16, segments the strands: the river's version of the menu bar's slats becoming dots, asked for because the level-only transcribing state was "not distinct enough". A first attempt with evenly spaced round beads was rejected as "just sort of weird"; the maintainer asked instead to keep the identical wave motion and morph each continuous line into three or four pulsing segments with natural, per-strand variation. Each strand keeps its exact wave and splits at breakpoints that differ per strand (three, four, and three segments; breaks at 0.38/0.68, 0.24/0.52/0.77, 0.31/0.63 of the width; gaps 0.04 to 0.055 of the width), each segment a sub-range of the same curve with round ends. The gaps open from zero over the 0.2 s crossfade, so the line morphs into the segments. Each segment's ink pulses between 60% and full on its strand's period (2.3, 2.0, 2.6 s), neighbours offset by 0.8 rad so the pulse flows downstream. Under Reduce Motion the segments hold at full ink. **Awaiting the maintainer's live read.** The mark renders 1:1 at 108 by 40.
...
**Turquoise variants, 2026-09-16.** The maintainer asked to see more saturated, cyan-leaning, even neon variants in case the brand color moves. Nine were built in OKLCH (one hue each; chroma and lightness vary), each as a light-desktop value chosen as the most vivid value at that hue that still carries text at 4.5:1 on white, and a dark-desktop value for charcoal. Finding: the light values converge near chroma 0.095 (`#00818C`, `#148284`, `#02837E` and so on, all about 4.6:1), because sRGB cannot hold a cyan that is both vivid and dark enough for text on white. The variants differ on charcoal: from the current `#3FA3AA` at 5.08:1 through `#3BCDDC` (7.9:1), `#00DDEF` (9.1:1), `#3FE4EC` (9.8:1) to `#14EEEE` (10.5:1). On any of the vivid dark values a pressed control needs charcoal text, since white on them fails. So a swap would change the dark-appearance accent and controls' text color, and barely change the light appearance. **Open; the maintainer is looking.**
...
| River transcribing as evenly spaced round beads riding the strands, with an ink crest or a downstream drift | "Beads are just sort of weird here"; replaced by the line morphing into a few pulsing segments per strand |
...
14. **Compute envelopes, don't draw them.** A bar that must hug a mark with a constant gap is the mark's offset (Minkowski sum with a disk), joined and closed with a fillet radius; `shapely` does this in a few lines and the result is exact.
