...
The rest state now is a calmer 0.5: amplitude and stroke width have a floor at the level-0.35 values (70% of 0.5's amplitude, stroke just under 0.5's width), the drift runs at 45% speed at rest and speeds up smoothly with level, and the ink dims to 44% with a slow breath of plus or minus 6% over about seven seconds (the breath's low point measures about 3.15:1 on charcoal; it holds still under Reduce Motion). Ink is the only thing that says "resting"; the water never stops. The amplitude floor is about 2.7 px on the thinnest strand, which keeps every strand out of the sub-pixel band that caused the grain. Ink follows the level through a smoothstep over the first half of the range, so on release it settles in step with the motion (the first build blended ink only below level 0.15, which the maintainer saw as a late, abrupt opacity step). Transcribing holds the level at 0.4 and, since 2026-09-16, beads the strands: the river's version of the menu bar's slats becoming dots, asked for because the level-only transcribing state was "not distinct enough". Beads sit every 4.5 px along each strand and ride its wave; strokes and beads crossfade over 0.2 s. Lead: the beads stay put and a crest of ink travels along them every 2.4 s, the same crest as the microphone indicator's dots. Alternate on the page: the beads drift downstream at 14 px per second, entering and leaving under the end fade so none pops. Under Reduce Motion the beads hold at full ink. **Awaiting the maintainer's read** of crest versus drift. The mark renders 1:1 at 108 by 40.
